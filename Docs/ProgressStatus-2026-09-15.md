...
- 캐릭터 1차 시안으로 `총총이`의 Idle·Attack·Death·Skill 전투 아트와 전용 투사체를 주인공에 연결했습니다.
- 동료·일반 적·보스에 공통 외곽선·그림자와 역할별 색상 등급을 적용했습니다.
...
- 기존 로컬 전투 아트와 런타임 UI를 사용했습니다. 이미지 생성 도구는 사용량 제한으로 실행할 수 없어 신규 원화 대신 기존 `총총이` 전투 아트를 1차 시안으로 연결했습니다.
