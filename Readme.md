...
현재 구현 범위, 검증 상태, 남은 작업은 [개발 진행 현황 — 2026-09-15](Docs/ProgressStatus-2026-09-15.md)을 기준으로 확인한다. 협업 기록은 [Notion 공동 개발 스페이스](https://www.notion.so/3a0a9c01e4068184b38ee28accfe5d64)에서 관리한다.
...
- 수치형 명중률·회피율은 없다. 일반 적 공격은 예고 위치에서 벗어나면 회피되고, 동료 스킬은 자동 명중한다.
- 플레이어는 전투 영역을 터치하거나 드래그해 참새 이등병을 이동시킨다. 적 공격 예고 중 이동하면 피해를 피할 수 있다.
...
- 챕터 내 일반 스테이지는 적이 순차 등장하는 웨이브 방식이다. `1~3`은 2마리, `4~6`은 3마리, `7~9`는 4마리다.
- 근접, 원거리 마법, 돌진 타입을 조합한다. 같은 타입은 최대 2회 연속 등장하며, `7~9`는 세 타입을 모두 포함한다.
- `10`배수 스테이지는 보스 스테이지다.
- 일반 고양이별 전투 수치와 웨이브 규칙은 `StageWaveDatabase`에서 관리한다. 런타임 전투 프로필은 `EnemyCombatProfile`을 사용한다.
...
- 장비 인벤토리는 5열, 최소 45칸, 세로 스크롤 구조다. 슬롯에 등급, 강화, 장비명, 착용 상태를 표시한다.
- 아이템 상세창은 아이콘, 등급, 종류, 이름, 랜덤 옵션, 해체 보상과 장착·해체 동작을 제공한다.
- 주요 장비 프리펩은 `Assets/Resources/Prefabs/UI`에 있다.
- `UI/Common`의 공용 프리펩 12종은 검수용으로 제작됐으며 기존 화면에는 아직 적용하지 않았다.
...
- 일반 적·웨이브 정의: `Assets/Resources/StageWaveDatabase.asset`
...
- 개편한 장비 인벤토리와 상세창을 Play Mode에서 검수한다.
- UI, 주인공, 동료, 일반 적, 보스의 공통 크기 조절 기준을 구현한다. 현재는 제안 단계다.
